...
- ✅ 6L (polimento 2026-05-12, branch `sprint-06-polish`) — Hero do desktop reescrito como split panel (texto esquerda + foto Pedro contida 4:5 à direita) — mobile mantém full-bleed. Header agora é `sticky top-0 z-40` com `bg-neutral-950/90 backdrop-blur` + ícones YouTube/Instagram. Integração canal YouTube: server-side fetch de `youtube.com/feeds/videos.xml` (sem API key, cache 1h) parseado por regex em `features/loja/youtube.ts`, com fallback `[]` em qualquer falha. **No desktop (Layout B — refinamento 6M)**, título+CTAs ocupam o topo full-width; abaixo, grid 2-col com foto à esquerda (aspect 4:5) e 3 `VideoRow` (thumb pequena + título 2 linhas + data) empilhados à direita — tudo numa única composição visual de hero. **No mobile**, hero segue full-bleed e os vídeos aparecem como seção standalone (`CanalYouTube` envolto em `md:hidden`) entre Destaques e Mais produtos. Card de vídeo extraído em `VideoCard` (canal-youtube.tsx) reaproveitado por ambos. Nova seção `MapaOficina` antes do footer com iframe Google Maps embed (`?output=embed`, sem API key) + CTA "Abrir no Google Maps" linkando a URL exata `share.google/C10nmYooVJJVGqW6c`. Footer ampliado pra 3 colunas (marca / oficina com endereço em São Luís → Google Maps / contato YT+IG+WA) + linha inferior `© ano PedroRed` + crédito "Feito por Vibe Surf Dev → vibesurfdev.com". Constantes centralizadas em `features/loja/contato.ts` (com helpers `getMapsUrl` e `getMapsEmbedUrl`). `YT_CHANNEL_ID` adicionado a `.env.local.template` e `docs/architecture/deploy.md`. Novo teste `features/loja/youtube.test.ts` (7/7) com fixture real do canal `@PEDROredtsi`.
